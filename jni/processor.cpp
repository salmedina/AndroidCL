//
// This is the interface to the kernel functions which are loaded
// into the devices
//
...
static cl::Kernel       gGaussianK;

// Reads the file contents and take it back a char buffer
...
JNIEXPORT jboolean JNICALL Java_com_example_LiveFeatureActivity_compileKernels(JNIEnv *env, jclass clazz)
{
    ...
    try {
    	// Seek the OpenCL capable devices
        std::vector<cl::Platform> platforms;
        cl::Platform::get(&platforms);
        //If no device found, nothing else to do
        if (platforms.size() == 0) {
            return false;
        }
        cl_context_properties properties[] =
        { CL_CONTEXT_PLATFORM, (cl_context_properties)(platforms[0])(), 0};
        //Grab the properties of the GPU's only
...
void helper(uint32_t* out, int osize, uint8_t* in, int isize, int w, int h, int choice)
{
    try {
        ...
        if (choice==1) {
        	...
        	gGaussianK.setArg(2,w);
        	gGaussianK.setArg(3,h);
        	gGaussianK.setArg(1,bufferOut);
        	gGaussianK.setArg(0,bufferOut2);
			gQueue.enqueueNDRangeKernel(gGaussianK,
					cl::NullRange,
					cl::NDRange( (int)ceil((float)w/16.0f)*16,(int)ceil((float)h/16.0f)*16),
					cl::NDRange(16,16),
					NULL,
...
JNIEXPORT void JNICALL Java_com_example_CameraPreview_runfilter(
        ...
        jint width,
        jint height,
        jint choice) //0: NV21 to RGB, 1: Laplacian Filter
{
